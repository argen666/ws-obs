...
def printMessage(jsondata):
    try:
        ...
        star_print(image)
        # image.save(str(time.time()) + 'final_image.png')
        ...
        for msg in message:
            ...
            if (type(msg) == dict and msg is not None):
                ...
                if ('iconURL' in msg and msg['iconURL'] is not None):
                    ...
                    mes = msg['text']
                    # print(msg)
                    image = text2png(mes, fontsize=25, fontfullpath=font)
                    if image is not None:
                        images.append(image)
                else:
